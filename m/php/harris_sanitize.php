 ...
 * @version     0.1
 */
class Harris_Sanitize
{
    // Singleton instance of harris
    ...
    {
        $this->lang = [
            'validate_required'                 => '{field} 字段必需输入',
            'validate_valid_email'              => '{field} 字段必须是有效的电子邮件地址',
            'validate_max_len'                  => '{field} 字段最多为 {param} 个字符',
            'validate_min_len'                  => '{field} 字段至少为 {param} 个字符',
            'validate_exact_len'                => '{field} 字段必须为 {param} 个字符',
            'validate_alpha'                    => '{field} 字段只能包含字母',
            'validate_alpha_numeric'            => '{field} 字段只能包含字母和数字',
            'validate_alpha_numeric_space'      => '{field} 字段只能包含字母，数字和空格',
            'validate_alpha_dash'               => '{field} 字段只能只包含字母和破折号',
            'validate_alpha_space'              => '{field} 字段只能包含字母和空格',
            'validate_numeric'                  => '{field} 字段必须是数字',
            'validate_integer'                  => '{field} 字段必须是不带小数的数字',
            'validate_boolean'                  => '{field} 字段必须是布尔类型',
            'validate_float'                    => '{field} 字段必须是带小数点的数字(float)',
            'validate_valid_url'                => '{field} 字段必须是URL',
            'validate_url_exists'               => '{field} 网址不存在',
            'validate_valid_ip'                 => '{field} 字段需要是一个有效的IP地址',
            'validate_valid_ipv4'               => '{field} 字段需要包含有效的IPv4地址',
            'validate_valid_ipv6'               => '{field} 字段需要包含有效的IPv6地址',
            'validate_guidv4'                   => '{field} 字段需要包含有效的GUID',
            'validate_valid_cc'                 => '{field} 不是有效的信用卡号码',
            'validate_valid_name'               => '{field} 应该是一个全名',
            'validate_contains'                 => '{field} 只能是以下之一： {param}',
            'validate_contains_list'            => '{field} 不是有效的选项',
            'validate_doesnt_contain_list'      => '{field} 字段包含不被接受的值',
            'validate_street_address'           => '{field} 字段需要是一个有效的街道地址',
            'validate_date'                     => '{field} 必须是有效的日期',
            'validate_min_numeric'              => '{field} 字段需要是数值，等于或高于 {param}',
            'validate_max_numeric'              => '{field} 字段需要是数值，等于或小于 {param}',
            'validate_min_age'                  => '{field} 字段需要具有大于或等于 {param} 岁的年龄 ',
            'validate_invalid'                  => '{field} 字段无效',
            'validate_starts'                   => '{field} 字段需要开始 {param}',
            'validate_extension'                => '{field} 字段只能包含以下扩展名之一： {param}',
            'validate_required_file'            => '{field} 字段必填项',
            'validate_equalsfield'              => '{field} 字段不等于 {param} 字段',
            'validate_iban'                     => '{field} 字段需要包含有效的IBAN',
            'validate_phone_number'             => '{field} 字段需要是有效的电话号码',
            'validate_regex'                    => '{field} 字段需要包含有效格式的值',
            'validate_valid_json_string'        => '{field} 字段需要包含一个有效的JSON格式字符串',
            'validate_valid_array_size_greater' => '{field} 字段需要是一个大小，等于或高于{param}的数组 ',
            'validate_valid_array_size_lesser'  => '{field} 字段需要是一个大小等于或小于{param}的数组',
            'validate_valid_array_size_equal'   => '{field} 字段需要是一个与{param}大小相等的数组 ',
        ];
    }
    /**
     ...
     * @param array $validators The harris validators
     *
     * @return mixed True(boolean) or the array of error messages
     */
    public static function is_valid(array $data, array $validators)
    {
        $harris = self::get_instance();
        $harris->validation_rules($validators);
        if ($harris->run($data) === false) {
            return $harris->get_readable_errors(false);
        } else {
            return true;
        }
    }
    ...
    {
        $harris = self::get_instance();
        return $harris->filter($data, $filters);
    }
    /**
    ...
    public function validate(array $input, array $ruleset)
    {
        ...
        foreach ($ruleset as $field => $rules) {
            $rules = explode('|', $rules);
            $lookFor = ['required_file', 'required'];
     ...
     * Usage:
     *
     * harris::set_error_messages(array(
     ...
     * @return string
     */
    public function get_readable_errors($convert_to_string = false, $field_class = 'harris-field', $error_class = 'harris-error-message')
    {
...
//                $message = str_replace('{param}', $param, str_replace('{field}', '<span class="'.$field_class.'">'.$field.'</span>', $messages[$e['rule']]));
    ...
    public function h_ack_message($arr =[])
    {
        if (empty($arr)) {
            return false;
        }
    ...
    public function h_ack_translate($arr=[],$str="")
    {
        if (empty($arr)&& empty($str)) {
            return false;
        }
        foreach ($arr as $key => $value) {
            $str=str_replace(ucfirst($key).' 字段',$value.' ',$str);
        }
        return $str;
    }
}
