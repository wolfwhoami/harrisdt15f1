 ...
 *   path="/wechat.php",
 *   tags={"存提款"},
 *   summary="在线存款--微信支付宝扫码",
 *   description="先调用接口(可判断是否可用):获取在线存款银行列表;返回结果<br>{pic_url:二维码图片地址,query_url:查询接口(有些第三方没有),qrcode_url:唤醒app地址(有些第三方不提供),status:succes,code:1000}<br>message 错误信息",
 ...
 *     name="bank_code",
 *     in="formData",
 *     description="扫码类型 微信用 weixin  支付宝用 zhifubao",
 *     required=true,
 *     type="string"
 *   ),
 *   @SWG\Response(
...
$params['bank_code'] = $params['bank_code']=='zhifubao' ? 'ZHIFUBAO' :'weixin';
;
...
if ($re['code'] == $objCode->success_save_online->code) {//在线存款底单提交成功
    ...
    $re['message'] = (json_encode($url_arr));
